...
#    name = name if fullpath else f'./{"user" if name in PREFS else "app"}/{name}.json'
    name = name if fullpath else f'{name}.json'
    try:
        with open(name) as f:
            return json.load(f)
    except FileNotFoundError:
        data = [] if PREFS[0] in name or PREFS[1] in name else {}
...
def jdump(data, name):
    try:
#        with open(f'./{"user" if name in PREFS else "app"}/{name}.json', 'w') as f:
